...
//import invaders.memento.GameObjectMemento;
...
public class Player implements Moveable, Renderable {
    ...
    private static class PlayerMemento implements Memento {
        public final Vector2D position;
        public final double health;
        PlayerMemento(Vector2D position, double health) {
            this.position = new Vector2D(position.getX(), position.getY());
            this.health = health;
            System.out.printf("Saving player pos (%f, %f)\n", this.position.getX(), this.position.getY());
        }

    }

    @Override
    public Memento save() { return new PlayerMemento(position, health); }

    @Override
    public void restore(Memento memento) {
        PlayerMemento playerMemento = (PlayerMemento) memento;
        this.position.setX(playerMemento.position.getX());
        this.position.setY(playerMemento.position.getY());
        System.out.printf("Restored player pos to (%f, %f)\n", playerMemento.position.getX(), playerMemento.position.getY());
