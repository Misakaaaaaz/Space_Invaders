...
//        System.out.println(projectileStrategy.toString());
        return projectileStrategy;
    }

    private static class EnemyMemento implements Memento {

        public int lives;
        public Vector2D position;
        public int xVel = -1;

        public EnemyMemento(int lives, Vector2D position, int xVel) {
            this.lives = lives;
    ...
    @Override
    public Memento save() { return new Enemy.EnemyMemento(lives, position, xVel); }

    @Override
    public void restore(Memento memento) {
        Enemy.EnemyMemento enemyMemento = (Enemy.EnemyMemento) memento;
        this.lives = enemyMemento.lives;
        this.position.setX(enemyMemento.position.getX());
        this.position.setY(enemyMemento.position.getY());
        this.xVel = enemyMemento.xVel;
    }
